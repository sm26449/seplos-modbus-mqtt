...
class MQTTManager:
    ...
    def _setup_client(self):
        ...
        # Set Last Will Testament - broker publishes this if we disconnect unexpectedly
        self.client.will_set(
            f"{self.prefix}/health/status",
            "offline",
            qos=1,
            retain=True
        )
    ...
    def _on_connect(self, client, userdata, flags, reason_code, properties=None):
        ...
        if reason_code == 0:
            ...
            # Publish online status (counterpart to LWT offline)
            self.client.publish(f"{self.prefix}/health/status", "online", qos=1, retain=True)
