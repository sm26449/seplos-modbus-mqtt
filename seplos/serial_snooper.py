...
class SerialSnooper:
    ...
    def __init__(self, port, mqtt_manager, mqtt_prefix, pack_aggregator, baudrate=19200):
        ...
        self.max_reconnect_delay = 60  # max delay with exponential backoff
    ...
    def _reconnect_serial(self):
        """Attempt to reconnect to serial port with exponential backoff"""
        while True:
            try:
                self.log.info(f"Attempting serial reconnect in {self.current_reconnect_delay}s...")
                ...
                self._connect_serial()
                self.log.info(f"Serial reconnected successfully to {self.port}")

                # Clear buffer after reconnect
                self.data = bytearray(0)
                return True

            except serial.SerialException as e:
                self.log.warning(f"Serial reconnect failed: {e}")
                # Exponential backoff
    ...
    def read_raw(self, n=256):
        ...
        Cu reconnect automat la erori de serial.
        """
        try:
            ...
            return b''  # Return empty to continue main loop
